...
- [x] **Transaction Categories** (Feature)
  - [x] Create `Category` model (`id`, `name`, `color`) in `src/budy/schemas.py`
  - [x] Add `category_id` Foreign Key to `Transaction` model
  - [x] Implement startup check in `src/budy/__init__.py` to safely add the column to existing DBs (SQLite `ALTER TABLE`)
  - [x] Add service functions: `create_category`, `delete_category`, `list_categories`
